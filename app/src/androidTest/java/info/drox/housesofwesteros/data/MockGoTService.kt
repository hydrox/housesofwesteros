...
class MockGoTService(
    ...
    override suspend fun listHouses(page: Int, pageSize: Int): List<House> {
        if (fails)
            throw HttpException(Response.error<List<House>>(404, ResponseBody.create(null, "")))
    ...
    override suspend fun getHouse(houseId: Int): House {
        if (fails || houseId < 1 || mockHouses.size < houseId)
            throw HttpException(Response.error<House>(404, ResponseBody.create(null, "")))
        return mockHouses[houseId - 1]
    }

    override suspend fun getCharacter(characterId: Int): Character {
        if (fails || characterId < 1 || mockCharacters.size < characterId)
            throw HttpException(Response.error<Character>(404, ResponseBody.create(null, "")))
